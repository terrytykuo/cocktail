...
for epo in range(epoch):
    for i, data in enumerate(mixloader, 0):
        ...
        if i % 50 == 0:
            ...
            cv2.imwrite(root_dir + 'cocktail/combinemodel_fullconv/L2/' + str(epo) + str(i)  + "_mix.png", inn)

            tarr = target.view(256, 128).detach().numpy() * 255
            np.clip(tarr, np.min(tarr), 1)
            cv2.imwrite(root_dir + 'cocktail/combinemodel_fullconv/L2/' + str(epo) + str(i)  + "_tar.png", tarr)

            outt = outputs.view(256, 128).detach().numpy() * 255
            np.clip(outt, np.min(outt), 1)
            cv2.imwrite(root_dir + 'cocktail/combinemodel_fullconv/L2/' + str(epo) + str(i)  + "_sep.png", outt)
